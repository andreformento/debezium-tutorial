...
case $1 in
    ...
    start-standalone)
        ...
        echo "BEFORE"
        ls $KAFKA_HOME/config/
        cp /connect-config/* $KAFKA_HOME/config/
        echo "AFTER"
        ls $KAFKA_HOME/config/
        cp -rn $KAFKA_HOME/config.orig/* $KAFKA_HOME/config
        echo "FINAL"
        ls $KAFKA_HOME/config/
        echo "##############"
        cat $KAFKA_HOME/config/connect-standalone.properties
        ...
        do
          ...
          if [[ $env_var =~ ^CONNECT ]]; then
            prop_name=`echo "$VAR" | sed -r "s/^CONNECT_(.*)=.*/\1/g" | tr '[:upper:]' '[:lower:]' | tr _ .`
            if egrep -q "(^|^#)$prop_name=" $KAFKA_HOME/config/connect-standalone.properties; then
                #note that no config names or values may contain an '@' char
                sed -r -i "s@(^|^#)($prop_name)=(.*)@\2=${!env_var}@g" $KAFKA_HOME/config/connect-standalone.properties
            else
                #echo "Adding property $prop_name=${!env_var}"
                echo "$prop_name=${!env_var}" >> $KAFKA_HOME/config/connect-standalone.properties
            fi
            if [[ "$SENSITIVE_PROPERTIES" = *"$env_var"* ]]; then
                echo "--- Setting property from $env_var: $prop_name=[hidden]"
            else
        ...
        #
        # Execute the Kafka Connect standalone service, replacing this shell process with the specified program ...
        #
        exec $KAFKA_HOME/bin/connect-standalone.sh $KAFKA_HOME/config/connect-standalone.properties $KAFKA_HOME/config/debezium.properties
        ;;
esac
